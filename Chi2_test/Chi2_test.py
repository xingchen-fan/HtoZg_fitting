...
def get_hist_data(x, CAT):
    """Retrieves sideband data histogram

    Args:
      x: fitting variable (RooAbsReal)
      CAT: category name

    Returns:
      sideband data RooDataHist 
    """
    # Define vars
...
def get_hist_data_pico(x, CAT, filename):
    """Retrieves sideband data histogram
    ...
    Returns:
      sideband data RooDataHist 
    """
    reader = readPico(x, filename)
    hist_data = getattr(reader, "data_hist_{}".format(CAT))
    return hist_data

def perform_sideband_chi2test(hist_data, x, CAT, setting):
    """Performs data sideband chi2 test with all functions

    Args:
      hist_data: RooDataHist of datasideband for performing fit
      x: fitting variable (RooAbsReal)
      CAT: category name
      setting: configuration parameter dictionary
    """
...
reader = None
if args.sample == "":
  hist_data = get_hist_data(x, CAT) #Cornell lxplus sample
else:
  hist_data = get_hist_data_pico(x, CAT, args.sample) #draw_pico sample
perform_sideband_chi2test(hist_data, x, CAT, setting)
