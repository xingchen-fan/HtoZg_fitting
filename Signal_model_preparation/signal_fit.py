...
DISIGMA = True
sig_model_el = DSCB_Class(x, MH_el,  CAT+"_"+args.year+"_el_"+args.prod, di_sigma = DISIGMA)
sig_model_mu = DSCB_Class(x, MH_mu,  CAT+"_"+args.year+"_mu_"+args.prod, di_sigma = DISIGMA)
...
if DISIGMA:
    ...
    note_mu = '#splitline{#splitline{#sigmaL = ' + '%.2f'%sig_model_mu.sigmaL.getVal() + '#pm%.2f'%sig_model_mu.sigmaL.getError()+ '}{nL = %.2f'%sig_model_mu.nL.getVal()+', nR = %.2f'%sig_model_mu.nR.getVal()+', #mu = %.2f'%MH_mu.getVal() +  '}}{#alphaL = %.2f'%sig_model_mu.alphaL.getVal() + ', #alphaR = %.2f'%sig_model_mu.alphaR.getVal()+'}'
plotClass(x, el_sig_hist, sig_model_el.pdf, sig_model_el.pdf, CAT+"_"+args.year+"_el_"+args.prod, note=note_el, CMS = "Simulation", fullRatio = True, leftSpace=True, bins = 2)
plotClass(x, mu_sig_hist, sig_model_mu.pdf, sig_model_mu.pdf, CAT+"_"+args.year+"_mu_"+args.prod, note=note_mu, CMS = "Simulation", fullRatio = True, leftSpace=True, bins = 2)
