...
if args.lep == '':
    ...
    dscb_model = combineSignalLep(x, MH, CAT, '../Config/config_DSCB.json', 'el')
elif args.lep == 'mu':
    dscb_model = combineSignalLep(x, MH, CAT, '../Config/config_DSCB.json', 'mu')
MH.setConstant(True)
...
file_ = 'workspaces/workspace_bkg_toy_unblind_'+CAT+'.root'

print("file = ", file_)
hist_toy = ROOT.readToyWs(file_, CAT, args.lep)
...
if DEBUG_SCAN :
    ...
    can1.BuildLegend()
    can1.Draw()
    can1.SaveAs("NLL_"+CAT + args.lep+".pdf")
    #######################################
