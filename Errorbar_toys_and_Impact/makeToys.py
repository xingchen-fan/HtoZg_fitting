...
if opt.batch == 'IC':
  ...
  # Create submission file
  fsub = open("./sub_toys_%s.sh"%opt.ext,'w')
  fsub.write("#!/bin/bash\n")
  fsub.write("cd ~/EOS_space/CMSSW_14_1_0_pre4/src/\n")
  fsub.write("cmsenv\n")
  # fsub.write("cd %s/src/flashggFinalFit/Plots/SplusBModels%s/toys\n"%(os.environ['CMSSW_BASE'],opt.ext))
  ...
  #fsub.write("eval `scramv1 runtime -sh`\n")
  fsub.write("cd ~/HtoZg_fitting/Make_combine_workspaces/condor/\n")
  fsub.write("itoy=$1\n\n")
  # Generate command
  fsub.write("#Generate command\n")
  gen_cmd = "combine %s -m %.2f -M GenerateOnly --saveWorkspace --toysFrequentist --bypassFrequentistFit -t 1 %s -s -1 -n _${itoy}_gen_step --freezeParameters MH"%(inputWSFile,mh_bf,setParamStr)
  if opt.loadSnapshot is not None: gen_cmd += " --snapshotName %s"%opt.loadSnapshot
  fsub.write("%s\n\n"%gen_cmd)
  # Fit cmd
  fsub.write("#Fit command\n")
  fsub.write("mv higgsCombine_${itoy}_gen_step*.root gen_${itoy}.root\n")
  fit_cmd = "combine gen_${itoy}.root -m %.2f -M MultiDimFit -P %s --floatOtherPOIs=1 --saveWorkspace --toysFrequentist --bypassFrequentistFit -t 1 %s -s -1 -n _${itoy}_fit_step --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2"%(mh_bf,opt.POIs.split(",")[0],setParamStr)
  fsub.write("%s\n\n"%fit_cmd)
  # Throw cmd
  fsub.write("#Throw command\n")
  fsub.write("mv higgsCombine_${itoy}_fit_step*.root fit_${itoy}.root\n")
  throw_cmd = "combine fit_${itoy}.root -m %.2f --snapshotName MultiDimFit -M GenerateOnly --saveToys --toysFrequentist --bypassFrequentistFit -t -1 -n _${itoy}_throw_step %s"%(mh_bf,setParam0Str)
  fsub.write("%s\n\n"%throw_cmd)
  # Clean up
  ...
  fcondor = open("./sub_toys_%s.sub"%opt.ext,'w')
  fcondor.write("executable = sub_toys_%s.sh\n"%opt.ext)
  fcondor.write("arguments = $(ProcId)\n")
  fcondor.write("output                = out_err/sub_toys.$(ClusterId).$(ProcId).out\n")
  fcondor.write("error                 = out_err/sub_toys.$(ClusterId).$(ProcId).err\n")
  fcondor.write("log                   = sub_toys.$(ClusterId).log\n\n")
  fcondor.write("MY.WantOS = \"el9\"\n")
  ...
  # Submission
  os.system("chmod 775 sub_toys_%s.sh"%opt.ext)
  ...
  #  else:
    os.system("condor_submit sub_toys_%s.sub"%opt.ext)
  else: print(" --> [DRY-RUN] jobs have not been submitted")
