...
Install combine v10.1.0. This can be done following the instructions [here](https://cms-analysis.github.io/HiggsAnalysis-CombinedLimit/latest/), however, before compiling with `scramv1 b` or similar, copy [this](Utilities/HZGRooPdfs.cxx) version of `HZGRooPdfs.cxx` and [this](Utilities/HZGRooPdfs.h) version of `HZGRooPdfs.h` over the versions in combine's `src` and `interface` directories and modify `src/classes_def.xml` to add `<class name="ModGaus" />` and `<class name="NormPow" />` under `<class name="RooStepBernstein" />`. If combine builds without errors, it should be compatible with the datacards generated by this repository.

 - Note that there are some strange bugs in combine/ROOT that cause conflicts in variable binning when using PDFs with `RooFFTConvPdf` that can cause failed fits or even segfaults in some cases. These only appear with the `-M FitDiagnostics` option of Combine, which should be avoided in this case
