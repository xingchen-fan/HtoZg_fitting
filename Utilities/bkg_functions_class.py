...
class Bern2RangeClass:
    def __init__(self, x, gauss_mu, cat="", p0=10, p1_init=0.3, p2_init=0.3, bond=20, sigma_init=7., step_init=105., fix_sigma = False, lowx = 100, highx = 165):
        ...
        self.sigma.setConstant(fix_sigma)
        self.stepval = ROOT.RooRealVar("bern2_step_" + cat, "bern2_step_" + cat, step_init, 90., 120.)
        self.pdf = ROOT.RooGaussStepBernsteinRange("bern2_" +cat + "_model", "Bernstein2 (X) gauss " + cat, x,  gauss_mu, self.sigma, self.stepval,lowx, highx, ROOT.RooArgList(self.p0,self.p1,self.p2))
        self.SBpdf = ROOT.RooGenericPdf("bern2_SB_" +cat + "_model", "((@0 < 120)? 1:((@0 > 130)? 1:0)) * @1", ROOT.RooArgList(x, self.pdf))
        self.name = "bern2_"+ cat
    def checkBond(self):
        tol = 0.001
...
class Bern3Class:
    def __init__(self, x, gauss_mu, cat="", p0=10, p1_init=0.3, p2_init=0.3, p3_init=0.3, bond=20, sigma_init=7., sigma2_init = 2, step_init=105., di_gauss = False, fix_sigma = False, gc_init = 0.5):
        ...
        self.sigma.setConstant(fix_sigma)
        self.sigma2.setConstant(fix_sigma)
...
class Bern3RangeClass:
    def __init__(self, x, gauss_mu, cat="", p0=10, p1_init=0.3, p2_init=0.3, p3_init=0.3, bond=20, sigma_init=7., step_init=105., fix_sigma = False, lowx = 100, highx = 165):
        ...
        self.sigma.setConstant(fix_sigma)
        self.stepval = ROOT.RooRealVar("bern3_step_" + cat, "bern3_step_" + cat, step_init, 90., 120.)
        self.pdf = ROOT.RooGaussStepBernsteinRange("bern3_" +cat + "_model", "Bernstein3 (X) gauss " + cat, x,  gauss_mu, self.sigma, self.stepval, lowx, highx, ROOT.RooArgList(self.p0,self.p1,self.p2,self.p3))
        self.SBpdf = ROOT.RooGenericPdf("bern3_SB_" +cat + "_model", "((@0 < 120)? 1:((@0 > 130)? 1:0)) * @1", ROOT.RooArgList(x, self.pdf))
        self.name = "bern3_"+ cat
    def checkBond(self):
        tol = 0.001
...
class Bern4RangeClass:
    def __init__(self, x, gauss_mu, cat="", p0=10, p1_init=0.3, p2_init=0.3, p3_init=0.3, p4_init=0.3, bond=20, sigma_init=7., step_init=105., fix_sigma = False, lowx = 100, highx = 165):
        ...
        self.sigma.setConstant(fix_sigma)
        self.stepval = ROOT.RooRealVar("bern4_step_" + cat, "bern4_step_" + cat, step_init, 90., 120.)
        self.pdf = ROOT.RooGaussStepBernsteinRange("bern4_" +cat + "_model", "Bernstein4 (X) gauss " + cat, x,  gauss_mu, self.sigma, self.stepval, lowx, highx, ROOT.RooArgList(self.p0,self.p1,self.p2, self.p3, self.p4))
        self.SBpdf = ROOT.RooGenericPdf("bern4_SB_" +cat + "_model", "((@0 < 120)? 1:((@0 > 130)? 1:0)) * @1", ROOT.RooArgList(x, self.pdf))
        self.name = "bern4_"+ cat
    def checkBond(self):
        tol = 0.001
...
class Bern5RangeClass:
    def __init__(self, x, gauss_mu, cat="", p0=1., p1_init=0.3, p2_init=0.3, p3_init=0.3, p4_init=0.3, p5_init=0.3, bond=20, sigma_init=7., step_init=105., fix_sigma = False, lowx = 100, highx = 165):
        ...
        self.sigma.setConstant(fix_sigma)
        self.stepval = ROOT.RooRealVar("bern5_step_" + cat, "bern5_step_" + cat, step_init, 90., 120.)
        self.pdf = ROOT.RooGaussStepBernsteinRange("bern5_" +cat + "_model", "Bernstein5 (X) gauss " + cat, x,  gauss_mu, self.sigma, self.stepval, lowx, highx, ROOT.RooArgList(self.p0,self.p1,self.p2, self.p3, self.p4, self.p5))
        self.SBpdf = ROOT.RooGenericPdf("bern5_SB_" +cat + "_model", "bern5_SB_" +cat + "_model", "((@0 < 120)? 1:((@0 > 130)? 1:0)) * @1", ROOT.RooArgList(x, self.pdf))
        self.name = "bern5_"+ cat
    def checkBond(self):
        tol = 0.001
...
class Pow1Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 3., step_init = 108., p_init = -6., p_low = -10., p_high = 0., di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.t = ROOT.RooRealVar("pow1_t_" + cat, "t pow1" + cat, step_init, 90., 120.)
        ...
        self.sigma.setConstant(fix_sigma)
        self.sigma2 = ROOT.RooRealVar("pow1_sigma2_" + cat,"pow1_sigma2_"+cat, sigma2_init,  0.1, 15.)
        ...
        self.gc = ROOT.RooRealVar("pow1_gc_" + cat, "pow1_gc_" + cat, gc_init, 0, 10)
        ...
        self.f = ROOT.RooRealVar("pow1_f_" + cat, "f pow1 " + cat, 0.1, 0., 10)
        self.step = ROOT.RooGenericPdf("pow1_step_" + cat, "pow1_step_" + cat,\
...
class Pow2Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 107., p1_init = -5.8, p1_low = -8., p1_high = 0., p2_init = -15, p2_low = -20., p2_high = -5., f1_init = 0.9, f2_init = 0.9, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.sigma.setConstant(fix_sigma)
        self.sigma2 = ROOT.RooRealVar("pow2_sigma2_" + cat,"pow2_sigma2_"+cat, sigma2_init,  0.1, 15.)
        self.sigma2.setConstant(fix_sigma)
        ...
        self.gc = ROOT.RooRealVar("pow2_gc_" + cat, "pow2_gc_" + cat, gc_init, 0, 10)
...
class Pow3Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 108., p1_init = -5.7, p1_low = -8., p1_high = 0., p2_init = -8., p2_low = -20., p2_high = -5., p3_init = -10., p3_low = -40., p3_high = -5., f1_init = 0.9, f2_init = 0.2, f3_init = 0.3, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.gc = ROOT.RooRealVar("pow3_gc_" + cat, "pow3_gc_" + cat, gc_init, 0, 10)
        ...
        self.step = ROOT.RooGenericPdf("pow3_step_" + cat, "pow3_step_" + cat, "( ((@0-@1)*307.69 <0.0) ? 0.0 : (((@0-@1)*307.69 >1.0) ? 1.0 : ((@0-@1)*307.69) ) )*( @5/ @8 *@0^@2 + @6/ @9 * @0^@3 + @7/ @10 * @0^@4)", ROOT.RooArgList(x,self.t,self.p1, self.p2, self.p3, self.f1, self.f2, self.f3, self.norm1, self.norm2, self.norm3))
...
class Exp1Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 108., p_init = -0.02, p_low = -2, p_high = 0., di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.sigma.setConstant(fix_sigma)
        self.sigma2 = ROOT.RooRealVar("exp1_sigma2_" + cat,"sigma2_exp1_"+cat, sigma2_init,  0.1, 15.)
        self.t = ROOT.RooRealVar("exp1_t_" + cat, "t exp1 " + cat, step_init, 90., 118.)
        self.p1 = ROOT.RooRealVar("exp1_p_" + cat, "p exp1 " +cat, p_init, p_low, p_high)
        ...
        self.gc = ROOT.RooRealVar("exp1_gc_" + cat, "gc_exp1_" + cat, gc_init, 0, 10)
...
class Exp2Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 105., p1_init = -0.02, p1_low = -1, p1_high = 0., p2_init = -0.02, p2_low = -1, p2_high = 0., f1_init = 0.5, f2_init = 0.5, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.step = ROOT.RooGenericPdf("exp2_step_" + cat, "exp2_step_" + cat, "( ((@0-@1)*153.85<0.0) ? 0.0 : (((@0-@1)*153.85 >1.0) ? 1.0 : ((@0-@1)*153.85) ) )*(@5 / @6 * TMath::Exp(@0*@2) + @4 / @7 * TMath::Exp(@0*@3))", ROOT.RooArgList(x,self.t,self.p1,self.p2,self.f1, self.f2, self.norm1, self.norm2))
        ...
        self.gc = ROOT.RooRealVar("exp2_gc_" + cat, "exp2_gc_" + cat, gc_init, 0, 10)
...
class Exp3Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7.,  step_init = 108., p1_init = -0.02, p1_low = -2, p1_high = 0., p2_init = -0.02, p2_low = -2, p2_high = 0., p3_init = -0.02, p3_low = -2, p3_high = 0., f1_init = 0.33, f2_init = 0.5, f3_init = 0.5, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.gc = ROOT.RooRealVar("exp3_gc_" + cat, "exp3_gc_" + cat, gc_init, 0, 10)
...
class Lau2Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 108., p1 = -7, p2 = -6, f1_init = 0.1, f2_init = 0.1, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.f2 = ROOT.RooRealVar("lau2_f2_" + cat, "f2 lau2 " + cat, f2_init, 0., 100.)
        ...
        self.gc = ROOT.RooRealVar("lau2_gc_" + cat, "lau2_gc_" + cat, gc_init, 0, 10)
...
class Lau3Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 108., p1 = -8, p2 = -7, p3 = -6, f1_init = 0.1, f2_init = 0.1, f3_init = 0.1, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.xmax = ROOT.RooRealVar("lau3_xmax_" + cat, "xmax_lau3_" + cat, xmax)
        self.offset = ROOT.RooRealVar("os_" + cat, "os_" + cat, 1e-5, 0., 0.01)
        self.f1 = ROOT.RooRealVar("lau3_f1_" + cat, "f1 lau3 " + cat, f1_init, 0., 20.)
        self.f2 = ROOT.RooRealVar("lau3_f2_" + cat, "f2 lau3 " + cat, f2_init, 0., 50.)
        self.f3 = ROOT.RooRealVar("lau3_f3_" + cat, "f3 lau3 " + cat, f3_init, 0., 50.)
        ...
        self.gc = ROOT.RooRealVar("lau3_gc_" + cat, "lau3_gc_" + cat, gc_init, 0, 10)
...
class Lau4Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., sigma2_init = 7., step_init = 108., p1 = -8, p2 = -7, p3 = -6, p4 = -5, f1_init = 0.1, f2_init = 0.1, f3_init = 0.1, f4_init = 0.1, xmax = 165., const_f1 = False, di_gauss = False, fix_sigma = False, gc_init = 1):
        ...
        self.xmax = ROOT.RooRealVar("lau4_xmax_" + cat, "xmax_lau4_" + cat, xmax)
        self.offset = ROOT.RooRealVar("os_" + cat, "os_" + cat, 1e-5, 0., 0.01)
        self.f1 = ROOT.RooRealVar("lau4_f1_" + cat, "f1 lau4 " + cat, f1_init, 0., 20.)
        self.f2 = ROOT.RooRealVar("lau4_f2_" + cat, "f2 lau4 " + cat, f2_init, 0., 20.)
        self.f3 = ROOT.RooRealVar("lau4_f3_" + cat, "f3 lau4 " + cat, f3_init, 0., 20.)
        self.f4 = ROOT.RooRealVar("lau4_f4_" + cat, "f4 lau4 " + cat, f4_init, 0., 20.)
        ...
        self.gc = ROOT.RooRealVar("lau4_gc_" + cat, "lau4_gc_" + cat, gc_init, 0, 10)
...
class Lau5Class:
    def __init__(self, x, gauss_mu, cat="", sigma_init = 7., step_init = 108., p1 = -8, p2 = -7, p3 = -6, p4 = -5, p5 = -4, f_init = 0.1, xmax = 165., const_f1 = False):
        ...
        self.sigma = ROOT.RooRealVar("lau5_sigma_" + cat,"sigma_lau5_" + cat       ,sigma_init,  1., 25.)
        self.xmax = ROOT.RooRealVar("lau5_xmax_" + cat, "xmax_lau5_" + cat, xmax)
        ...
        self.step = ROOT.RooGenericPdf("lau5_step_" + cat, "lau5_step_" + cat, "( ((@0-@1)*153.85<0.0) ? 0.0 : (((@0-@1)*153.85 >1.0) ? 1.0 : ((@0-@1)*153.85) ) )*(@2/@12*@0^@7 + @3/@13*@0^@8 + @4/@14*@0^@9 + @5/@15*@0^@10 + @6/@16*@0^@11)", \
...
class ModGausClass:
    def __init__(self, x, cat="", lowx = 105., highx = 170., m0 = 105, sl = 1, sh = 40, vl = 2, vr = 1):
        self.m0 = ROOT.RooRealVar("modg_m0_"+cat,"mass peak value [GeV]" , m0,95,130)
        self.xlow = ROOT.RooRealVar("modg_xlow_"+cat,"xlow modg" , lowx)
        self.xhigh = ROOT.RooRealVar("modg_xhigh_"+cat,"xhigh modg" , highx)
        ...
        self.width = ROOT.RooFormulaVar("modg_width_"+cat, "@1 + @2*(@0 - @3)/(@4-@3)", ROOT.RooArgList(x, self.sl, self.sh, self.xlow, self.xhigh))
