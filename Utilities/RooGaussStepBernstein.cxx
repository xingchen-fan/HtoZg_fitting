...
ClassImp(RooGaussStepBernsteinRange)
...
RooGaussStepBernsteinRange::RooGaussStepBernsteinRange()
{
}


//_____________________________________________________________________________
RooGaussStepBernsteinRange::RooGaussStepBernsteinRange(const char* name,
					     const char* title,
					     RooAbsReal& x,
					     RooAbsReal& mean,
					     RooAbsReal& sigma,
  ...
  _coefList("coefficients","List of coefficients",this)
{
  ...
  while((coef = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      cout << "RooGaussStepBernsteinRange::ctor(" << GetName()
...
//_____________________________________________________________________________
RooGaussStepBernsteinRange::RooGaussStepBernsteinRange(const RooGaussStepBernsteinRange& other, 
					     const char* name) :
...
//_____________________________________________________________________________
Double_t RooGaussStepBernsteinRange::evaluate() const
{  
  ...
  for( int i = 0; i <= degree; ++i ) {
    ...
    // covolution
    poly_vals[i] = poly_conv(_x,_mean,_sigma,_stepVal,_highx*1.10, i);
  }
...
//_____________________________________________________________________________
Int_t RooGaussStepBernsteinRange::getAnalyticalIntegral(RooArgSet& allVars,
						   RooArgSet& analVars,
						   const char* rangeName) const
{  
  //if (matchArgs(allVars, analVars, _x) && _coefList.getSize() <= 6) return 1;
  // For plotting over a different range, disable analytical integral
  return 0;
}


//_____________________________________________________________________________
Double_t RooGaussStepBernsteinRange::analyticalIntegral(Int_t code,
						   const char* rangeName) const
{
  assert(code==1) ;
  ...
  for( int i = 0; i <= degree; ++i ) {
    // we stretch the upper bound by 10% to remove the turn-off of 
    // covolution
    integral_vals_lo[i] = poly_conv_integral(_x.min(),_mean,_sigma,_stepVal,
					     _x.max(), i);
    integral_vals_hi[i] = poly_conv_integral(_x.max(),_mean,_sigma,_stepVal,
					     _x.max(), i);
  }
