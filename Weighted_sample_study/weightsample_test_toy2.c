...
// Toy2 script, explicit weight function: fitTo and chi2FitTo, two options, please choose as intended. Please remember chi2FitTo doesn't have asymptotic error option!
...
//w(x) = 0.4x + 0.5
    RooGenericPdf line3("line3", "line3", "0.4*@0 + 0.5", RooArgList(x));
    RooFormulaVar wFunc("wFunc","event weight","0.5/(0.4*x + 0.5)",x);
//w(x) = 0.9x^2 + 0.2
    // RooGenericPdf line3("line3", "line3", "0.9*@0*@0 + 0.2", RooArgList(x));
    // RooFormulaVar wFunc("wFunc","event weight","0.5/(0.9*x*x + 0.2)",x);


    // RooGenericPdf line3("line3", "line3", "0.9*@0*@0 + 0.5*@0 + 0.2", RooArgList(x));
    // RooFormulaVar wFunc("wFunc","event weight","0.5/(0.9*x*x + 0.5*x + 0.2)",x);

    // RooGenericPdf fitline("fitline", "fitline", "@1*@0*@0 + @2", RooArgList(x, c, e));
    RooGenericPdf fitline("fitline", "fitline", "@1*@0 + 0.5", RooArgList(x, c));
    
    TH1D *h_sumw2 = new TH1D("hsumw2", "hsumw2", 100, -5., 5.);
    TH1D *h_offsumw2 = new TH1D("hoffsumw2", "hoffsumw2", 100, -5., 5.);
    TH1D *h_asym = new TH1D("hasym", "hasym", 100, -5., 5.);
    TH1D *h_dummy = new TH1D("hdummy", "hdummy", 100, -5., 5.);
    
    TH1D *h_dummy_cen = new TH1D("hdummy_cen", "hdummy_cen", 100, -0.05, 0.05);
    TH1D *h_weight_cen = new TH1D("hweight_cen", "hweight_cen", 100, -0.05, 0.05);
    ...
    int N = 10000;
    for (int i(0); i < N; i++){
      int N1 = (int)generator->Gaus(20000, 141);
//w(x) = 0.4x + 0.5
        auto data1 = line3.generate(x, NumEvents(N1));
        RooRealVar *w = (RooRealVar*) data1->addColumn(wFunc);
        RooDataSet wdata1("wdata1","wdata1",data1,*data1->get(),0,w->GetName());
