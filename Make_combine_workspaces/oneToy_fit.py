...
ROOT.gInterpreter.Declare("""
RooDataHist readToyWs(TString filename, string CAT, string model){
auto file_ = new TFile(filename, "READ");
RooWorkspace *ws = (RooWorkspace *)file_->Get("workspace_bkg");
RooDataHist *oneToy  = (RooDataHist *)ws->data(("hist_" + model + "_" + CAT).c_str());
return *oneToy;
}
""")
...
profile_class = profileClass(x, mu_gauss, CAT, args.config)
...
file_ = 'workspaces/workspace_bkg_profile_bias_asimov_'+CAT+'.root'

print("file = ", file_)
hist_toy = ROOT.readToyWs(file_, CAT, args.func)
