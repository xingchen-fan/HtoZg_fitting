...
RooDataHist* mlly_plot_generated_blinded_window(RooRealVar &m_lly, RooDataHist hist_sideband_rdh, RooAbsPdf *signal_fit, RooAddPdf *background_fit, int nbackground, double nsignal, ostream &out_file, int scale_factor=1, bool isAsimov=false){
...
void signal_extraction_blinded_region(string infile_one, string infile_two, string outfile, string func, string category, double signal_boost=1.0, int ntrials=100){
  ...
  ofstream out_file; out_file.open("./siginjection_csv/csv_" + category + "_sig_strength_" + to_string(static_cast<int>(signal_boost)) + "_extended.csv");
  ...
  for(int idx = 1; idx <= ntrials; idx++){
    sideband_fit = fit_sideband(m_lly, hist_sideband_rdh, nev_sideband, func, m_lly_low, m_lly_high, nbins, category + "_sideband_fit");
    ...
    RooDataHist full_range_with_bdata_two = *(mlly_plot_generated_blinded_window(m_lly, hist_sideband_rdh, signal_fit_model, sideband_fit, nev_sideband, nsig_trial, out_file));
    ...
    //Every hundredth fit plot the result 
    if(idx%100==0){
...
int signal_extraction_with_generated_data(string category, double signal_strength){
  ...
  for(string func : func_list){
    string out_plot_name = "./output/test_sig_ext_blinded_" + category + "_" + func + "_sigstr_" + to_string(static_cast<int>(signal_strength*100)) +"percent"; 
    signal_extraction_blinded_region(file_path + plot_one, file_path + plot_two, out_plot_name, func, category + "_" + func, signal_strength, toys);
  }
